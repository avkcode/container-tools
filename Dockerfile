...
ARG TARGETARCH
ARG TARGETVARIANT
ENV TARGETOS=${TARGETOS} TARGETARCH=${TARGETARCH} TARGETVARIANT=${TARGETVARIANT}
...
RUN set -eux; \
    ...
    curl -fsSL https://github.com/sigstore/cosign/releases/latest/download/cosign-linux-${TARGETARCH} -o /usr/local/bin/cosign; \
    chmod +x /usr/local/bin/cosign; \
    curl -fsSL https://storage.googleapis.com/container-structure-test/latest/container-structure-test-linux-${TARGETARCH} -o /usr/local/bin/container-structure-test; \
